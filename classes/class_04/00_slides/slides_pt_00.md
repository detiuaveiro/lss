...
## O que é uma Página Web "Estática"? i
  ...
  * Tecnologias centrais: **[HTML](https://en.wikipedia.org/wiki/HTML)** (para a estrutura) e **[CSS](https://en.wikipedia.org/wiki/CSS)** (para o estilo).

## O que é uma Página Web "Estática"? ii

![\ ](static-website.png){ width=100% }

## HyperText Markup Language i

Vamos decompor isto:

  * **HyperText (Hipertexto):** Refere-se a texto que contém links ("[hyperlinks](https://en.wikipedia.org/wiki/Hyperlink)") para outros textos (ou páginas). Este é o conceito fundamental que conecta toda a web.
  * **Markup Language (Linguagem de Marcação):** Isto *não* é uma linguagem de programação.
      * Uma **[linguagem de programação](https://en.wikipedia.org/wiki/Programming_language)** *faz* coisas (ex: `if (x > 5) { do_something() }`).
      * Uma **[linguagem de marcação](https://en.wikipedia.org/wiki/Markup_language)** *descreve* coisas. Usa "tags" (etiquetas) para definir a estrutura e o significado do conteúdo.

## HyperText Markup Language ii

```html
<!-- Isto NÃO é programação. É uma descrição. -->
<!-- Diz: "Este texto é um cabeçalho de nível um." -->
<h1>Este é o meu título principal</h1>

<!-- Diz: "Este texto é um parágrafo." -->
...
<html lang="en">
  ...
  <body>
      ...
      <footer>
        <p>LSS DETI UA, All rights reserved</p>
      </footer>
  </body>
</html>
```

## A Evolução para HTML5 i

### HTML Antigo (HTML4):

Só tínhamos um contentor genérico: `<div>`. 
Isto levava a código que era difícil de ler, tanto para programadores como para máquinas (como leitores de ecrã ou motores de busca).
...
![\ ](figures/html-vs-html5.png){ width=85% }

## Porque é que a Semântica HTML5 é Importante i
...
## Porque é que a Semântica HTML5 é Importante ii

2. **[Otimização para Motores de Busca (SEO):](https://en.wikipedia.org/wiki/Search_engine_optimization)**
...
## Organizar Conteúdo: `<article>` vs. `<section>` i
...
## Organizar Conteúdo: `<article>` vs. `<section>` ii
...
## Media HTML5: Imagens `<img>` ii

```html
<img src="images/my-photo.jpg" alt="legenda de uma foto">

<img src="gifs/funny-reaction.gif" alt="gif animado">

<img src="https://placehold.co/400" alt="placehold.co">
```

## Media HTML5: Áudio `<audio>` i
...
## O "S": Seletores & Propriedades (Selectors & Properties) 
...
## Dinâmica do Lado do Cliente: `fetch()` i
...
## Dinâmica do Lado do Cliente: `fetch()` ii
...
## Dinâmica em Tempo Real: WebSockets i
...
## Dinâmica em Tempo Real: WebSockets ii
...
* É um protocolo **publicar/subscrever (pub/sub)** extremamente leve, popular para dispositivos **[IoT](https://en.wikipedia.org/wiki/Internet_of_things)** (Internet das Coisas).
* **Modelo Pub/Sub:**
...
## Como é que um Browser Acede *Realmente* a uma Página i
...
## Como é que um Browser Acede *Realmente* a uma Página ii
...
## Como é que um Browser Acede *Realmente* a uma Página iii
...
## Como é que um Browser Acede *Realmente* a uma Página iv
...
## Como é que um Browser Acede *Realmente* a uma Página v

12. **Pedidos Subsequentes:** O browser lê o `<head>` e encontra: `<link rel="stylesheet" href="style.css">`
...
## Exemplo Nginx + Docker Compose 

Este ficheiro `docker-compose.yml` é a receita para o seu servidor:
...
## Porque Usar um Reverse Proxy? i
...
## Porque Usar um Reverse Proxy? ii
...
## Porque Usar um Reverse Proxy? iii
...
## O que é o [CORS](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing)?
...
## Porque é que a [SOP](https://en.wikipedia.org/wiki/Same-origin_policy) existe?
