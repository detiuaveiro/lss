...
async def chat_handler(websocket, path):
  connected_clients.add(websocket) # Register new client
  print(f"Client connected: {websocket.remote_address}")
  try:
    async for message in websocket: # Iterate over messages
...
# Start a background thread for handling network
client.loop_start() 
```
